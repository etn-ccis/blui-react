...
| minHeight             | Minimum height of the table container                                                                          | `string \| number`                                                 | no       | `'100px'`         |
...
`DataTableColumnDef<TData>` extends MRT's `MRT_ColumnDef<TData>` with one additional convenience prop. All standard MRT column options (including `Cell`, `muiTableBodyCellProps`, `muiEditTextFieldProps`, `editVariant`, `editSelectOptions`, etc.) are fully supported.
