...
describe('R2L', () => {
    ...
    it('should display R2L menu RTL', () => {
        cy.get('[data-cy=change-language]').click();
        cy.contains('Arabic').click();
        cy.get('[data-cy=R2L-menu]').invoke('prop', 'offsetLeft').should('be.lessThan', 5);
    });

    it('should display language selector RTL', () => {
        cy.get('[data-cy=change-language]').click();
        cy.contains('Arabic').click();
        cy.get('[data-cy=change-language]').invoke('prop', 'offsetLeft').should('be.greaterThan', 500);
    });
